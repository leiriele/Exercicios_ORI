...
for filename in os.listdir('Docs'):
    if filename.endswith('.pdf'):
        ...
        words = [inflector.singular_noun(stemmer.stem(word)) or word for word in words]
...
for term, postings in sorted(terms_dict.items()):
    ...
    postings_list = ', '.join([f'({docid}, {freq})' for docid, freq in postings])
...
file_path = 'Relatorios/Relatorio_Indice_Invertido.txt'
...
with open('Relatorios/Relatorio_Indice_Invertido.txt', 'a') as arquivo_saida:
    for term, postings in terms_dict.items():
        freq = len(postings)
        arquivo_saida.write('Termo "{}" encontrado em {} documento(s):\n {}\n\n'.format(term, freq, postings))

# Salvar as Stopwords em um relatorio txt 
file_path = 'Relatorios/Relatorio_Stopwords.txt'
# Verificar se o arquivo ainda não existe
if not os.path.exists(file_path):
    with open(file_path, 'w+') as f:
        f.write('❶ Stopwords ❶\n\n')
...
with open('Relatorios/Relatorio_Stopwords.txt', 'a') as f:
    for word in stop_words:
        f.write(word + '\n')

# Salvar o índice invertido em um relatorio XLSX
index_df.to_excel('Relatorios/Relatorio_Indice_Invertido_Tabela.xlsx', index=False)